...
export class ID3Engine {
  ...
  /**
   * Identifies and scrubs embedded AI generation metadata watermarks (e.g. Suno, Udio tags)
   * from audio file ID3 header frames and replaces them with user-supplied custom tags.
   */
  public async scrubAITags(filePath: string, customTags?: Partial<ID3Tags>): Promise<ID3Tags> {
    const currentTags = await this.readTags(filePath);
    
    // AI keyword filters
    const aiPattern = /\b(suno|udio|ai-generated|made with suno|created with suno|generated by suno|suno\.ai|udio\.com)\b/i;

    const sanitized: ID3Tags = { ...currentTags };

    if (sanitized.artist && aiPattern.test(sanitized.artist)) {
      sanitized.artist = customTags?.artist || 'Unknown Artist';
    }
    if (sanitized.publisher && aiPattern.test(sanitized.publisher)) {
      sanitized.publisher = customTags?.publisher || '';
    }
    if (sanitized.composer && aiPattern.test(sanitized.composer)) {
      sanitized.composer = customTags?.composer || '';
    }
    if (sanitized.comment && aiPattern.test(sanitized.comment)) {
      sanitized.comment = customTags?.comment || '';
    }
    if (sanitized.album && aiPattern.test(sanitized.album)) {
      sanitized.album = customTags?.album || 'Single Release';
    }

    // Merge in explicit custom tags if provided
    if (customTags) {
      if (customTags.artist) sanitized.artist = customTags.artist;
      if (customTags.album) sanitized.album = customTags.album;
      if (customTags.genre) sanitized.genre = customTags.genre;
      if (customTags.year) sanitized.year = customTags.year;
      if (customTags.publisher) sanitized.publisher = customTags.publisher;
      if (customTags.composer) sanitized.composer = customTags.composer;
      if (customTags.comment) sanitized.comment = customTags.comment;
      if (customTags.artwork) sanitized.artwork = customTags.artwork;
    }

    // Write sanitized tags back to audio file
    await this.writeTagsToAudioFile(filePath, sanitized);
    return sanitized;
  }
...
export const scrubAITags = (filePath: string, customTags?: Partial<ID3Tags>) => defaultEngine.scrubAITags(filePath, customTags);
